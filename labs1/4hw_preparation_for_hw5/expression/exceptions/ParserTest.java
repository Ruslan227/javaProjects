...
public class ParserTest extends BaseTest {
    ...
    protected ParserTest() {
        ...
        levels.add(list(op("+", (a, b) -> a + b), op("-", (a, b) -> a - b)));
        levels.add(list(op("*", (a, b) -> a * b), op("/", (a, b) -> b == 0 ? error(DBZ) : a / b)));

        tests = list(op("10", (x, y, z) -> 10L), op("x", (x, y, z) -> x), op("y", (x, y, z) -> y),
                op("z", (x, y, z) -> z), op("x+2", (x, y, z) -> x + 2), op("2-y", (x, y, z) -> 2 - y),
                op("  3*  z  ", (x, y, z) -> 3 * z), op("x/  -  2", (x, y, z) -> -x / 2),
                ...
                op("x--y--z", (x, y, z) -> x + y + z), op("((2+2))-0/(--2)*555", (x, y, z) -> 4L),
    ...
    private void check(final int[] vars, final TripleExpression expression, final Either<Reason, Integer> answer) {
        ...
        try {
            ...
            assertEquals(String.format("f(%d, %d, %d)\n%s", vars[0], vars[1], vars[2], expression), answer.getRight(),
                    actual);
        } catch (final Exception e) {
            if (answer.isRight()) {
                throw new AssertionError(
                        String.format("No error expected for x=%d, y=%d, z=%d", vars[0], vars[1], vars[2]), e);
            }
        }
        counter.passed();
    }
    ...
    private Test genExpression(final int depth, final int coefficient, final int[] vars, final int level) {
        if (level == levels.size()) {
            ...
            return binary(levels.get(level), genExpression(depth + 1, coefficient, vars, level),
                    genExpression(depth, coefficient, vars, level + 1));
        } else {
    ...
    private Test binary(final List<Op<LongBinaryOperator>> ops, final Test t1, final Test t2) {
        ...
        return new Test(t1.expr + " " + op.name + " " + t2.expr,
