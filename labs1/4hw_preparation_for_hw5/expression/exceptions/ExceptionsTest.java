...
public class ExceptionsTest extends ParserTest {
    ...
    protected final List<Op<String>> parsingTest = list(op("No first argument", "* y * z"),
            op("No middle argument", "x *  * z"), op("No last argument", "x * y * "),
            op("No first argument'", "1 + (* y * z) + 2"), op("No middle argument'", "1 + (x *  / 9) + 3"),
            op("No last argument'", "1 + (x * y - ) + 3"), op("No opening parenthesis", "x * y)"),
            op("No closing parenthesis", "(x * y"), op("Start symbol", "@x * y"), op("Middle symbol", "x @ * y"),
            op("End symbol", "x * y@"), op("Constant overflow 1", Integer.MIN_VALUE - 1L + ""),
            op("Constant overflow 2", Integer.MAX_VALUE + 1L + ""), op("Bare +", "+"), op("Bare -", "-"),
            op("Bare a", "a"), op("(())", "(())"), op("Spaces in numbers", "10 20"));
    ...
    protected Either<Reason, Integer> lift(final long value) {
        return value < Integer.MIN_VALUE || Integer.MAX_VALUE < value ? Either.left(OVERFLOW) : super.lift(value);
    }
}
